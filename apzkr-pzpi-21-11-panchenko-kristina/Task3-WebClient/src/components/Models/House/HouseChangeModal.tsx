...
import { editHouse } from '../../../http/houseApi';
...
export const HouseChangeModal = ({ show, onHide, item, fetch }: IProps) => {
    const {
        control,
        handleSubmit,
        reset,
        formState: { errors },
      } = useForm<IHouseChangeData>();
      ...
      const onSubmit = async (data: IHouseChangeData) => {
        await editHouse(data.id, data)
          .then(() => {
            onHide();
            fetch();
          })
